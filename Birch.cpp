...
Birch Birch::fit(xt::xarray<float> X, void* y=NULL) {
...
Birch Birch::_fit(xt::xarray<float> X, bool partial) {
    ...
    for (auto sample : X) { 
        std::vector<int> mol_indices{this->index_tracker};
        _CFSubcluster subcluster = _CFSubcluster(sample, mol_indices);
        _CFNode self = *this->root_;
        bool split = self.insert_cf_subcluster(&subcluster);
