...
int main() {
    ...
    //     oss << std::setw(10) << n << "    " << std::fixed << std::setprecision(6) << std::setw(10) << std::setfill('0') << std::setfill(' ') << (time(NULL) - v) << std::endl;
    //     z += oss.str();
    // }
...
xt::xarray<float> calc_centroid(xt::xarray<float> linear_sum, int n_samples, int sim_index = 'JT') { // linear_sum is the same as c_total
    // Calculates centroid
    if (sim_index == 'JT') {
...
std::pair<_CFSubcluster*, _CFSubcluster*> _split_node(_CFNode* node, double threshold, int branching_factor) {
    ...
    _CFSubcluster temp1 = _CFSubcluster();
    _CFSubcluster temp2 = _CFSubcluster();
    _CFSubcluster* new_subcluster1 = &temp1;
    _CFSubcluster* new_subcluster2 = &temp2;
    _CFNode temp3 = _CFNode(
        threshold=threshold, 
    ...
    _CFNode* new_node1 = &temp3;
    _CFNode temp4 = _CFNode(
        threshold=threshold, 
        branching_factor=branching_factor, 
        node->is_leaf, 
        node->n_features //----maybe private these attributes and use getters?
